...
def _synapse_counts_row(cols,i,m, assume_0_index = False):
    ...
    for j in range(len(cols.Assigned_columns[i])):
        ...
        if cols.Assigned_columns[i][j] == -1:
            ...
            ind = cols.Assigned_columns[i][j]
            if not assume_0_index:
                
            row_data[ind] += cols.Synapse_counts[i][j]
    return row_data

def synapse_count_matrix(cols,assignment_df, assume_0_index = False):
