...
from numpy import where, isnan, array
from typing import List
from Neurosetta import Forest_graph, Tree_graph, get_all_centres, get_all_roots

from .columns import Columns
from .pnt_assignment import max_pnt_assignment, vmf_likelihood_matrix
from .col_assignment import hungarian_tie_handling
...
def Neuron_column_assignment(cols: Columns, N_all: Forest_graph, point_type:str = 'root', bind: bool = True) -> List | None:
    ...
    # set nan values to 0
    mat[where(isnan(mat))] = 0
    # make assignment list
    a_list = hungarian_tie_handling(mat.T)
    # mark unassigned as -1
    a_list = [array([v,-1]) if isnan(a_list[v]).any() else array(a_list[v]) for v in N_all.graph.iter_vertices()]
    # bind or return dict
    if bind:
        if point_type == 'root':
        ...
        for v in N_all.graph.iter_vertices():
            assignment = a_list[v][1]
            N_all.graph.vp['Neurons'][v].graph.gp[gp_label] = N_all.graph.new_gp('int',assignment[1])
    else:
        return a_list