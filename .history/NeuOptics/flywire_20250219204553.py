...
def flywire_column_assignment_table():
    """Load the column coordinates data file from the installed package."""
    try:
        with pkg_resources.files(NeuOptics).joinpath("data/N_id.column_assignments.csv").open("r") as f:
            return pd.read_csv(f)
    except FileNotFoundError:
        raise FileNotFoundError("Data file 'N_id_column_assignments.csv' not found in the installed package. "
                                "Ensure NeuOptics is installed correctly and includes the 'data' folder.")
