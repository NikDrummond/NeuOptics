...
def _fit_sphere(coords: np.ndarray)  :
    """ Helper function to fit a sphere to a point cloud using vedo"""
...
def _N_sphere_projection(N: Union[np.ndarray, nr.Tree_graph], sphere: Union[vd.Sphere, None] = None, r: Union[float, None] = None, c:Union[np.ndarray, None] = None, bind: bool = False) -> np.ndarray | None:
    """ Helper function for projecting a single neurons points onto the surface of a sphere"""
...
def project_to_sphere(
    ...
    """
    Given either a List of neurons, a single neuron, or a Forest graph, project all involved points onto a sphere.

    The sphere is defined by either giving this function a vd.Sphere object - as returned by
    the fit_sphere function, or the radius and center of mass can be given as r and c.

    If input Neurons have input and output synapse tables, these will also be transformed

    Note - you CAN use this with bind = False, but the output is a pain and this is not advised

    If however if you insist, If the neurons passed to this function do not have inputs and outputs, the output here is quite simple.

    Simply, The vertex coordinates of the given neuron(s) will be returned as an np.ndarray

    However, if you do have input and output properties on your neurons what is returned is 5 arrays per neuron.

    The order of these arrays is: vertices, inputs which are not mapped to the skeleton, inputs which are mapped to the skeleton,
    outputs which are not mapped to the skeleton and finally outputs which are mapped to the skeleton.

    If only inputs or outputs are available, then only the one which is available will be returned

    In the case of multiple neurons, and inputs and outputs being available, a list of length n * 5 arrays will be returned.

    If you insist on not using bind = True, managing the output is on you!
    """
    ...
    if isinstance(N, List):
        ...
        return [_N_sphere_projection(n, sphere, r, c, bind) for n in N]

    # if this is a single tree graph
    elif isinstance(N, nr.Tree_graph):
        return _N_sphere_projection(N, sphere, r, c, bind)
    ...
    # if this is a Forest graph object
        ...
        for v in N.graph.iter_vertices():
            ...
            if ~bind:
                data.append(_N_sphere_projection(n, sphere, r, c, bind))
            else:
                _N_sphere.projection(n, sphere, r, c, bind)